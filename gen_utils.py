...
def prepare_inputs_for_generation(model, *args, **kwargs):
...
def _prepare_model_inputs(
    ...
    if input_name == "input_ids" and "inputs_embeds" in model_kwargs:
        if not model.config.is_encoder_decoder:
            ...
            if not has_inputs_embeds_forwarding:
                raise ValueError(
                    f"You passed `inputs_embeds` to `.generate()`, but the model class {model.__class__.__name__} "
                    "doesn't have its forwarding implemented. See the GPT2 implementation for an example "
                    "(https://github.com/huggingface/transformers/pull/21405), and feel free to open a PR with it!"
                )
            # In this case, `input_ids` is moved to the `model_kwargs`, so a few automations (like the creation of
            # the attention mask) can rely on the actual model input.
            model_kwargs["input_ids"] = model._maybe_initialize_input_ids_for_generation(
    ...
    inputs = model._maybe_initialize_input_ids_for_generation(inputs, bos_token_id, model_kwargs)
    return inputs, input_name, model_kwargs

def adjust_logits_during_generation(model, logits: torch.FloatTensor, **kwargs) -> torch.FloatTensor:
    """
...
def _get_decoder_start_token_id(model, decoder_start_token_id: int = None, bos_token_id: int = None) -> int:
    decoder_start_token_id = (
        ...
        else model.generation_config.decoder_start_token_id
    )
...
def _extract_past_from_model_output(model, outputs: ModelOutput, standardize_cache_format: bool = False):
    ...
    if standardize_cache_format and hasattr(model, "_convert_to_standard_cache"):
        batch_size = outputs.logits.shape[0]
        past_key_values = model._convert_to_standard_cache(past_key_values, batch_size=batch_size)
...
def _update_model_kwargs_for_generation(
    ...
    model_kwargs["past_key_values"] = model._extract_past_from_model_output(
...
def _reorder_cache(model, past_key_values, beam_idx):
    raise NotImplementedError(
        f"Make sure that a `_reorder_cache` function is correctly implemented in {model.__class__.__module__} to"
        f" enable beam search for {model.__class__}"
    )
...
def _get_logits_processor(
    ...
    processors = model._merge_criteria_processor_list(processors, logits_processor)
    # `LogitNormalization` should always be the last logit processor, when present
    if generation_config.renormalize_logits is True:
...
def _get_stopping_criteria(
    model, generation_config: GenerationConfig, stopping_criteria: Optional[StoppingCriteriaList]
) -> StoppingCriteriaList:
    ...
    criteria = model._merge_criteria_processor_list(criteria, stopping_criteria)
    return criteria
...
def compute_transition_scores(
    ...
    >>> tokenizer = outputs.sequences[:, input_length:]
    >>> for tok, score in zip(tokenizer[0], transition_scores[0]):
...
def _validate_model_class(model):
    """
    ...
    if not model.can_generate():
        ...
        for model_mapping in generate_compatible_mappings:
            supported_models = model_mapping.get(type(model.config), default=None)
            if supported_models is not None:
                generate_compatible_classes.add(supported_models.__name__)
        exception_message = (
            f"The current model class ({model.__class__.__name__}) is not compatible with `.generate()`, as "
...
def _validate_model_kwargs(model, model_kwargs: Dict[str, Any]):
